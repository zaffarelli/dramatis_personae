...
class Combattant(models.Model):
    ...
    def prepare_for_battle(self):
        ...
        self.round_data = {
            ...
            'weapon':{'id':w.id,'name':w.weapon_ref.reference,'DC':w.weapon_ref.damage_class,'WA':w.weapon_ref.weapon_accuracy},
            'REF':0,
            'AGI':0,
            'melee':0,
            'dodge':0,
        ...
        self.tell('<u>%s</u> uses his/her <b>%s</b>, granting himself/herself an accuracy bonus of <b>%d</b>, for a damage class of <b>%s</b>.'%(self.full_name,self.peek('weapon.name'),self.peek('weapon.WA'),self.peek('weapon.DC')))
    ...
    def roll_attack(self,target):
        if self.peek('number_of_attacks')>0:
            ...
            self.poke('attack_roll',self.peek('REF')+self.peek('melee')+self.peek('weapon.WA')-self.peek('health_template.circumstance_modifiers')-self.peek('multiattack_malus')+self.peek('health_template.expertise_bonus')+die)
            sum = self.peek('REF')+self.peek('melee')+self.peek('weapon.WA')-self.peek('health_template.circumstance_modifiers')-self.peek('multiattack_malus')+self.peek('health_template.expertise_bonus')
            ...
            self.poke('attack_sequence',"<u>"+self.full_name+"</u> <b>"+attk_order+" attack !<br/></b> REF + Melee + WA - CM - MA + EX -> "+x+str(sum)+"+"+detdie+"=<b>"+str(self.peek('attack_roll'))+"</b>")
            ...
            if self.peek('attack_roll')>target.peek('defender_dodge_roll'):
                self.poke('damage',fs_fics7.roll_dc(self.peek('weapon.DC'))+self.SA_DMG+fs_fics7.roll_dc("%dD6"%(overrun_bonus)))
                target.tell('<u>%s</u> is hit by %s for <b>%d</b> hit points...'%(target.full_name,self.full_name,self.peek('damage')))
                self.tell('<u>%s</u> rolls for damage: %s + %d + %dD6 = <b>%d</b> hit points...'%(self.full_name,self.peek('weapon.DC'),self.SA_DMG,overrun_bonus,self.peek('damage')))
            else:
                self.poke('damage',0)
                if tgt_parry:
            ...
            if true_damage >= self.peek('health_template.shield.min'):
                if self.peek('health_template.shield.charges')>0:
                    if true_damage <= self.peek('health_template.shield.max'):
                        ...
                        effect_self = '<u>%s</u> attack is <b>partially blocked</b> by an energy shield...'%(source.full_name)
                        effect_source = 'After shield block, upcomming damage is %d'%(true_damage)
                        self.poke_inc('health_template.shield.charges',-1)
                else:
                    true_damage = damage
    ...
    def armor_deflect(self,damage,source,where):
        ...
        effect_self = '<u>%s</u> armor blocks %d damage...'%(self.full_name,self.peek('health_template.'+where+'.SP'))
    ...
    def localize_damage(self,damage,source,where):
        ...
        if (where == 'HEAD'):
            true_damage *= 2
            effect_self = '<u>%s</u> attack lands on the <b>%s</b> of <u>%s</u> for double damage!'%(source.full_name,where,self.full_name)
            effect_source = 'After localisation check, upcomming damage is %d'%(true_damage)
        else:
            effect_self = '<u>%s</u> attack lands on the <b>%s</b> of <u>%s</u>...'%(source.full_name,where,self.full_name)
            effect_source = 'After localisation check, upcomming damage is %d'%(true_damage)
        self.tell(effect_self)
        source.tell(effect_source)
        return true_damage
    ...
    def absorb_punishment(self,source):
        ...
        if damage>0:
            ...
            if true_damage>0:
                self.tell('After protection checks, <u>%s</u> loses only <b>%s</b> hp...'%(self.full_name,true_damage))
                source.tell('...')
