...
from collector.forms.basic import CharacterForm, TourOfDutyFormSet, CharacterTodForm
...
class CharacterUpdateTodView(AjaxFromResponseMixin, UpdateView):
    model = Character
    form_class = CharacterForm
    context_object_name = 'c'
    template_name_suffix = '_update_tod_form'
    success_url = 'recalc_avatar'
    ...
    def get_context_data(self, **kwargs):
        context = super(CharacterUpdateTodView, self).get_context_data(**kwargs)
        if self.request.POST:
            context['form'] = CharacterTodForm(self.request.POST, instance=self.object)
            context['tourofdutys'] = TourOfDutyFormSet(self.request.POST, instance=self.object)
            context['tourofdutys'].full_clean()
            messages.success(self.request, 'Avatar updated: %s' % (context['form']['full_name'].value()))
        else:
            context['form'] = CharacterTodForm(instance=self.object)
            context['tourofdutys'] = TourOfDutyFormSet(instance=self.object)
            messages.info(self.request, 'Avatar displayed: %s' % (context['form']['full_name'].value()))
        return context
