...
' + flist + base_svg + '</svg>';
    ...
    drawWatermark(page = 0) {
        ...
        if (page === 0) {
            ...
            me.daddy = me.lines;
            let basex=1, basey=1, row_max=4, delta=0.15, cardw=4.5, cardh=6;
            _.forEach(me.data,function(e,i){
                let ox = (i%row_max)*(cardw+1)+basex, oy = Math.floor(i/row_max)*(cardh+1)+basey;
                me.drawLine(ox-delta, ox+cardw+delta, oy, oy, me.draw_fill, 3);
                me.drawLine(ox-delta, ox+cardw+delta, oy+cardh, oy+6, me.draw_fill, 3);
                me.drawLine(ox, ox, oy-delta, oy+6+delta, me.draw_fill,  1);
                me.drawLine(ox+cardw, ox+cardw, oy-delta, oy+cardh+delta, me.draw_fill, 1);

                me.drawText(ox + cardw/2, oy+ 0.5, me.draw_fill, me.shadow_stroke, me.medium_font_size*0.9, "middle", e["full_name"]);
                me.drawText(ox + cardw/2, oy+ 1.0, me.draw_fill, me.shadow_stroke, me.medium_font_size*0.8, "middle", e["alliance"]);
                me.drawText(ox + cardw/2, oy+ 5.8, me.draw_fill, me.shadow_stroke, me.small_font_size, "middle", e["entrance"]);
                me.drawText(ox + cardw/2, oy- 0.15, me.draw_fill, me.shadow_stroke, me.small_font_size, "middle", "DE AUTOMATUM LEGIS");
                me.fillPicture(ox,oy,e["rid"]);
                me.drawCircle(0.4, "", ox+cardw, oy+2*cardh/5,3,)
                me.drawCircle(0.4, "", ox+cardw, oy+3*cardh/5,3)
                me.drawCircle(0.4, "", ox+cardw, oy+4*cardh/5,3)
            });
    ...
    fillPicture(basex, basey,rid) {
        let me = this;
        let imglnk = 'media/images/f_' + rid + ".jpg";
        console.log(imglnk)
        me.daddy.append("svg:image")
            .attr("xlink:href", function (d) {
                return imglnk;
            })
            .attr("x", (basex+1.0) * me.stepx)
            .attr("y", (basey+1.25) * me.stepy)
            .attr("width", 0.8*3 * me.stepx)
            .attr("height", 0.8*4 * me.stepx)
        ;
    }
    ...
    perform() {
        let me = this;
        console.log('EPIC_DECK: Performing...');
        me.drawWatermark();
