...
def index(request):
	latest_character_list = Character.objects.order_by('-pub_date')
...
def edit_persona(request,id=None):
	item = get_object_or_404(Character, id=id)
	form = PersonaForm(request.POST or None, instance = item)
	if form.is_valid():
		form.save()
	return render(request, 'collector/persona_form.html', {'form': form})
